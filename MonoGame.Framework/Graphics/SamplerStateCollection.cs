    ...
    public sealed class SamplerStateCollection
	{
        ...
        internal void SetSamplers(GraphicsDevice device)
        {
            ...
            for (var i = 0; i < _samplers.Length; i++)
            {
                ...
                if (texture != null)
                {
                    var lastSamplerState = texture.glLastSamplerState[i];

                    if (sampler != null && texture != null && sampler != lastSamplerState && (lastSamplerState == null || sampler.Compare(lastSamplerState)))
                    {
